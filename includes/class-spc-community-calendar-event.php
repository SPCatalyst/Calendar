...
class SPC_Community_Calendar_Event {
	...
	public function get_formatted_datetime() {
		$ds   = $this->get_start_date( 'ymd' );
		$de   = $this->get_end_date( 'ymd' );
		$ds_f = $this->get_start_date( 'M d @ h:i A' );
		$de_f = $this->get_end_date( 'M d @ h:i A' );
		$hs   = $this->get_start_date( 'h:i A' );
		$he   = $this->get_end_date( 'h:i A' );
		
		if ( $ds_f === $de_f ) {
			$date = $ds_f;
		} else if ( $ds === $de ) {
			$mday = $this->get_start_date( 'M d' );
			$date = sprintf( '%s @ %s - %s', $mday, $hs, $he );
		} else {
			if ( ! empty( $ds_f ) && ! empty( $de_f ) ) {
				$date = sprintf( '%s - %s', $ds_f, $de_f );
			} else if ( ! empty( $ds_f ) ) {
				$date = $ds_f;
			} else {
				$date = '';
